...
- `corepack pnpm native:guard` prueft den gebauten Produktionspfad auf Public-HTML-Schatten, alte Baseline-/Componentized-Routenverzeichnisse, Public-`legacy-*` Assets, rohe Legacy-Render-Marker, unerwartete Runtime-`node:fs`-Zugriffe, fehlende native Layout-Marker und fail-closed Dispatch fuer adoptierte Routen. `production:check` fuehrt diesen Guard nach dem Web-Build aus.
