(function(){
  ...
  (function(){
    ...
    var firstImg = slides[0] && slides[0].querySelector('.hero-mr__image');
    ...
    var slideReady = [];
    function ensureSlideReady(idx){
      if(!slideReady[idx]){
        var imgs = Array.prototype.slice.call(slides[idx].querySelectorAll('img'));
        slideReady[idx] = Promise.all(imgs.map(decodeImage));
      }
      return slideReady[idx];
    }
    function idle(callback, timeout){
      if('requestIdleCallback' in window) requestIdleCallback(callback, { timeout: timeout });
      else setTimeout(callback, Math.min(timeout, 1400));
    }
    ...
    function nextSlide(){
      ...
      ensureSlideReady(idx).then(function(){ if(!document.hidden) activateSlide(idx); });
    }
    function jump(idx){
      if(idx === current) return;
      ensureSlideReady(idx).then(function(){ activateSlide(idx); });
    }
    ...
    function startCycle(){
      if(reduce || timer || cycleBootTimer) return;
      cycleBootTimer = setTimeout(function(){
        cycleBootTimer = null;
        idle(function(){
    ...
    if(firstImg){
      ensureSlideReady(0).then(reveal);
